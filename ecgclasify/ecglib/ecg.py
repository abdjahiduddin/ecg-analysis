...
def getFiltered(signal=None, sampling_rate=360.):
    # ensure numpy
    signal = np.array(signal)
...
def  getSummary(signal=None, sampling_rate=360.):
    # ensure numpy
    signal = np.array(signal)
    ...
    # final heart rate 
    heart_rate = []
    for count, i in enumerate(hr):
        heart_rate.append([ts_hr[count], i])
    
    # templates
    templates_tp = templates.T.transpose()
    hr_template = []
    for x in templates_tp:
        tmp = []
        for count, y in enumerate(x):
            tmp.append([ts_tmpl[count],y])
        hr_template.append(tmp)
        
    return rpeaks, heart_rate,hr_template

def  getTemplate(signal=None, sampling_rate=360.):
    # ensure numpy
    signal = np.array(signal)
